#ifndef EXPLANATIONS_H_
#define EXPLANATIONS_H_

/*
	This document will contain only text information about ways chosen for library implementation

	1. SYSCLK frequency could be changed only by reprogramming. So no dynamic changes are allowed. The main reason is the big amount of time-dependent functions because of control theory implementations.
	 	 Every SYSCLK change should be carefully traced in all time-related setups.
	 	 Only frequencies multiple to 2 Mhz are allowed.

	2. All peripherals sets up in different functions. It makes code bigger, but easier to comment, understand and maintain.




	Прблемы:
	* Не предусмотрена никакая система оповещения периферии о том, что PLL стал и не может быть запущен. И соответственно если такое происходит, то вся ссистема будет настроена неправильно.
		Можно исправить добавив в дополнение макроса глобальную переменную, которая бы хранила частоту макроса при инициализации, но могла бы быть изменена при ошибках с часами.
		Именно по этой переменной тогда должны настриваться часы. При этом надо иметь функцию, которая бы перенастроила все часы так чтобы заданные параметры соответствовали уже новой процессорной частоте.
	* Лог в современной реализации содержит только коды ошибок, но не содержит информацию об успешных операциях, что в общем случае тоже было бы полезно для восстановления общей картины мира при неисправностях
	* Нет никакого функционала, которы позволял бы определять скорость включения системы и выполнения отдельных ее функций.


	Протестировано:

	* Гибкая настройка частоты SYSCLK с шагом в 2Мгц работает как с HSI, так и с HSE
	* Переключение с HSI на HSE при отключении HSE работает. Частота работы сохраняется
	* Гибкая настройка частоты шима сохраняется
	* Гибкая настройка частоты UART работает в зависимости от частоты процессора
	* Лог ошибок работает - время записывается, как и коды ошибок, запись начинается с нуля при переполнении


	* Гибкая настройка частоты прерывания SysTick работает
	* Гибкая безопасная настройка частоты SPI работает

	Непротестировано:
	* Переключение на HSI при проблемах с PLL (не уверен, что вообще может такое произойти и тем более не знаю, как это протестировать)

 */





#endif /* EXPLANATIONS_H_ */
